...
const CustomerHomeScreen = () => {
  ...
  const [query, setQuery] = useState(""); // Initial query as empty string
  const [results, setResults] = useState([]);
  const [error, setError] = useState("");
  ...
  const handleSearch = async (searchQuery = query) => {
    ...
    try {
      ...
      if(query === ""){
        ...
        url = `http://localhost:8081/api/customers/search?query=${encodeURIComponent(searchQuery)}&latitude=${26.692284424260034}&longitude=${88.3774020141318}`;
      }
  ...
  // Call handleSearch on component mount and when query changes
  useEffect(() => {
    handleSearch(query);
  }, [query]);
